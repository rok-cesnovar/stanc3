...
let semantic_check_profile_start ~loc ~cf ps =
  Validate.(
    ps
    |> List.map ~f:(semantic_check_printable cf)
    |> sequence
    |> map ~f:(fun ups ->
           mk_typed_statement ~stmt:(ProfileStart ups) ~return_type:NoReturnType ~loc
       ))

let semantic_check_profile_end ~loc ~cf ps =
  Validate.(
    ps
    |> List.map ~f:(semantic_check_printable cf)
    |> sequence
    |> map ~f:(fun ups ->
           mk_typed_statement ~stmt:(ProfileEnd ups) ~return_type:NoReturnType ~loc
       ))
...
and semantic_check_statement cf (s : Ast.untyped_statement) :
  ...
  | ProfileStart ps -> semantic_check_profile_start ~loc ~cf ps
  | ProfileEnd ps -> semantic_check_profile_end ~loc ~cf ps
